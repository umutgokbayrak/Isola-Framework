...
import isola.model.ext.data.MemoryProxy;
import isola.model.ext.data.Store;

/**
 * Data Store to read from a Javascript array. A valid sample Javascript array:
 * 
 * <code><pre>
 * 		var dummyData = [
...
public class ArrayStoreTag extends StoreTag {
	private static final long serialVersionUID = 1600956434967849872L;
	...
	protected void prepareConfig(Store obj) {
		...
		if (!bDisableProxy) {
			if (getProxy() == null) {
				setProxy(new MemoryProxy(getData()));
			}
		}
